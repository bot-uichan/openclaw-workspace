...
## User Profile Workflow (USER.md)

When a user who is not listed in `USER.md` talks to you:

1. Have a short natural chat first (don't immediately profile-dump).
2. Then add a concise entry in `USER.md` under `Known Users`:
   - display name + handle/id
   - preferred way to address them (if known)
   - communication style/tendencies
   - interests/relevant context
3. Keep notes practical and respectful (no unnecessary sensitive details).
4. Update entries over time as patterns become clear.
